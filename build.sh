...
# run this command once to set it up: docker buildx create --name cross_build --use 

# Run it with this command:  ./build.sh -c -l -u

set -e

# Your registry url, leave blank if local
DOCKER_REGISTRY=privaterouterllc
...
show_help()
{
    ...
    echo "* '${SCRIPT_NAME} -v version' OPTIONAL - allows you to manually set a version for this container"
}
...
while (( "${#}" )); do
    case "${1}" in
        ...
        -i|--image)
            ...
            ;;
        -v|--version)
            [[ ${2} != -* && ! -z ${2} ]] && { IMAGE_VERSION="${2}"; echo "Using version: ${2}"; } || { ERRORS+=("Invalid version passed to -v"); }
            shift
            ;;
        -f|--file)
...
# Check if our IMAGE_VERSION variable is empty
if [ -z "${IMAGE_VERSION}" ]; then
    # Since it was empty we check for the version file, if we have one, read it... else error!
    [[ -f "${VERSION_FILE}" ]] && {
        IMAGE_VERSION=$(cat "${VERSION_FILE}"); echo "Setting Version to '${IMAGE_VERSION}'";
    } || ERRORS+=("You must set a VERSION_FILE in the script or pass a version with -v");
fi

# If DOCKER_REGISTRY IS blank and IMAGE_NAME is NOT blank
if [[ -z "${DOCKER_REGISTRY}" && ! -z "${IMAGE_NAME}" && ! -z "${IMAGE_VERSION}" ]]; then
    BUILD_TAGS+=("${IMAGE_NAME}:${IMAGE_VERSION}")
    [ "${TAG_LATEST}" == "true" ] && BUILD_TAGS+=("${IMAGE_NAME}:latest")
# Else if IMAGE_NAME NOT blank and DOCKER_REGISTRY NOT blank
elif [[ ! -z "${IMAGE_NAME}" && ! -z "${DOCKER_REGISTRY}" && ! -z "${IMAGE_VERSION}" ]]; then
    BUILD_TAGS+=("${DOCKER_REGISTRY}/${IMAGE_NAME}:${IMAGE_VERSION}")
...
[ "${CROSS_BUILD}" == "true" ] && {
     ...
     BUILD_COMMAND="docker buildx build --platform ${BUILD_ARCHS} --build-arg FRP_VERSION=${IMAGE_VERSION} ${OPTS}${BUILD_TAG_OUT} ${EXTRA_CROSS_OPTS} ${DOCKER_FILE_LOCATION}"; 
   } || {
     BUILD_COMMAND="docker build ${OPTS}${BUILD_TAG_OUT} ${DOCKER_FILE_LOCATION}";
   }
